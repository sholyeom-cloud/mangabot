...
SERPAPI_KEY = os.getenv("SERPAPI_KEY")  # optional for auto image search
...
def make_text_image_fullscreen(text: str, width=WIDTH, height=HEIGHT, font_path=None, font_size=64, bg_color=(10, 10, 10)):
    ...
    for line in lines:
        ...
        w0 = bbox[2] - bbox[0]
        draw.text((int((width - w0) / 2) + 2, y + 2), line, font=font, fill=(0, 0, 0))
        draw.text((int((width - w0) / 2), y), line, font=font, fill=(255, 255, 255))
        y += line_h + 8
    return img
...
def send_email_with_video(video_path: Path, title: str, slide_meta: list):
...
def build_video():
    ...
    if cat_clip:
        components.append(cat_clip.subclip(0, TITLE_DURATION))
    comp = CompositeVideoClip(components, size=(WIDTH, HEIGHT)).set_duration(TITLE_DURATION)
    if tts_title.exists():
        comp = comp.set_audio(AudioFileClip(str(tts_title)))
    clips.append(comp)
    ...
    for title, desc in selected:
        ...
        if cat_clip:
            comps.append(cat_clip.subclip(start, end))
        composed = CompositeVideoClip(comps, size=(WIDTH, HEIGHT)).set_duration(PER_ITEM_DURATION)
        if tts_path.exists():
            composed = composed.set_audio(AudioFileClip(str(tts_path)))
    ...
    if tts_out.exists():
        composed = composed.set_audio(AudioFileClip(str(tts_out)))
    clips.append(composed)
    ...
    # send email
    send_email_with_video(out_file, title_text, slide_meta)
